...
export default function UserComp({user, status}: {user: DBUser | null, status: string}){
    ...
    return(
        ...
        <section className="flex flex-col w-full p-8 space-y-8">
            ...
            className="font-ibarra  font-bold  text-secondary-foreground text-3xl lg:text-5xl">{`Profil: ${user?.name} ${user?.surname}`}</motion.h2>
            ...
            {!user?.isMailSub && user?.raynet_id && user?.id &&
