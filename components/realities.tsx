...
export default function Realities({ realities }: { realities: Reality[] }) {
    return (
        <div className="grid grid-cols-1 sm:grid-cols-3  w-full mx-auto lg:w-3/4 gap-5 ">
            {realities?.length > 0 ? (
                realities.map((r, i) => (
                    <div key={i} className="w-full border border-secondary-foreground transition ease-in-out hover:shadow-lg hover:shadow-secondary-foreground h-96 flex flex-col hover:z-10 hover:rounded-[15px] hover:p-3 hover:scale-125 hover:animate-card bg-white border-2">
