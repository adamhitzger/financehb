...
export default async function ArticlePage({ params }: { params: { slug: string } }) {
    ...
    return (
        <main className="flex min-h-screen flex-col space-y-3 p-8">
            <h2 className="mt-10  text-left font-medium tracking-wide text-secondary text-3xl">{new Date(article.datum).toISOString().split("T")[0]} / {article.name}</h2>
            <article className={`w-full flex flex-row border-t-2 border-t-secondary`}>
                ...
                <div className={`hidden w-1/2 sm:flex flex-col justify-center border-l-2 border-l-secondary text-right text-xl  py-12 px-5 font-light space-y-2 lg:p-10`}>
                    <PortableText value={article.overview} components={components} />
                </div>
